...
from phiseg.utils import show_tensor
        ...
        for i in range(self.resolution_levels):
            ...
            self.contracting_path.append(
                DownConvolutionalBlock(
                    input, output, initializers, depth=3, padding=padding, pool=pool, reversible=reversible
                )
            )
        ...
        for i in reversed(range(self.latent_levels)):
            ...
            if i == self.latent_levels - 1:
                ...
                self.sample_z_path.append(
                    SampleZBlock(
                        input, depth=2, reversible=reversible
                    )
                )
            else: 
                self.sample_z_path.append(
    ...
    def forward(self, patch, segm=None, training_prior=False, z_list=None):
        if segm is not None:
            ...
            with torch.no_grad():
                ...
                segm_one_hot = utils.convert_batch_to_onehot(segm, nlabels=2).to(device)
...
class PHISeg(nn.Module):
    ...
    def forward(self, patch, mask, training=True):
        if training:
            ...
            # print('posterior latent space shape:', self.posterior_latent_space[4].shape)
    ...
    def KL_two_gauss_with_diag_cov(self, mu0, sigma0, mu1, sigma1):
        # should return the correct stuff
    ...
    def calculate_hierarchical_KL_div_loss(self):
        # this guy should return kl loss and not the toal loss!!
    ...
    def residual_multinoulli_loss(self, reconstruction, target):
        ...
        # this guy here should return recon loss and not total loss
        ...
        for ii, s_ii in zip(reversed(range(self.latent_levels)),
            ...
            if ii == self.latent_levels-1:
                ...
                # if ii == 0:
                    ## for debugging 
                    # s_ii_copy = torch.clone(s_ii)
                    # s_ii_copy = s_ii_copy.detach().cpu().numpy().reshape((128,128))
                    # plt.imshow(s_ii_copy)
                    # plt.show()
                    ##
            ...
            recon_loss += self.residual_multinoulli_loss_weight * self.loss_dict['residual_multinoulli_loss_lvl%d' % ii]
        return recon_loss

    def generate_fast_samples(self, x, n):
        bs = x.shape[0]
        ...
        accumulated = accumulated.view([n, bs] + [*accumulated.shape][1:])
        del(prior_latent_space)
        del(s_out_list)
        del(xb)
        return accumulated
    ...
    def predict(self,x, n):
        with torch.no_grad():
            if torch.cuda.is_available():
                self.cuda()
                x = x.cuda()
            mean = torch.mean(self.generate_fast_samples(x, n), dim=0)
    ...
    def loss(self, segm):
        ...
        # for debugging
        # s_out_copy = self.s_out_list
        # fig, axs = plt.subplots(1,5, figsize=(20,7))
        # for i in range(5):
        #     curr_copy = s_out_copy[i].detach().cpu().numpy()[0,1,:,:]
        #     axs[i].imshow(curr_copy)
        # plt.show()


        self.loss_tot = torch.add(self.reconstruction_loss, self.kl_divergence_loss_weight * self.kl_divergence_loss)
...
def compute_train_loss_and_train(train_loader, model, optimizer, use_gpu):
    ...
    epoch_loss = running_loss / 12078# len(train_loader.dataset)
    epoch_kl_loss = kl_running_loss / 12078# len(train_loader.dataset)
    epoch_recon_loss = recon_running_loss / 12078# len(train_loader.dataset)

    return epoch_loss, epoch_kl_loss, epoch_recon_loss

def compute_eval_loss(test_loader, model, use_gpu):
    """
    ...
    epoch_loss = running_loss / 1509# len(train_loader.dataset)
    epoch_kl_loss = kl_running_loss / 1509# len(train_loader.dataset)
    epoch_recon_loss = recon_running_loss / 1509# len(train_loader.dataset)

    return epoch_loss, epoch_kl_loss, epoch_recon_loss

def train_model(model, train_loader, eval_loader, optim, epochs=1, wandb_name=None, save_model=None, save_path=None, continue_training_path=None):
    """
    ...
    for epoch in range(epochs):
        ...
        train_running_loss, train_kl_loss, train_recon_loss = compute_train_loss_and_train(train_loader, model, optim, use_gpu)

        # compute evaluation loss
        eval_running_loss, eval_kl_loss, eval_recon_loss = compute_eval_loss(eval_loader, model, use_gpu)

        # compute GED between samples and GTs on evaluation set
        if epoch % 10 == 0:
            mean_ged = utils.eval_ged(model, eval_loader, n_samples=10)
        ...
        if save_model==True:
            if eval_running_loss < best_total_eval_loss:
                ...
                torch.save({
                    ...
                    }, f'phiseg_best_eval_epoch{epoch}.pth')
                # torch.save(model.state_dict(), save_path +  f'phiseg_best_eval_epoch{epoch}.pth')
                print('saving best eval model')
            if epoch % 10 == 0:
                if mean_ged < best_ged_eval_loss:
                    best_ged_eval_loss = mean_ged
                    torch.save({
                    'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optim.state_dict(),
                    'loss': train_running_loss,
                    }, f'phiseg_best_ged_epoch{epoch}.pth')
